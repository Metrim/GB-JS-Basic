...
const config = {
    ...
    getRefreshCounter() {
        return this.settings.refreshCounter;
    },
    setRefreshCounter(value) {
        this.settings.refreshCounter = value;
    },
...
const obstacles = {
    ...
    init(numberObstacle = 1) {
        for (let i = 0; i < numberObstacle; i++) {
            this.elements.push(this.obstacle)
        }        
    }
}
...
const game = {
    ...
    renderScore(score = 1) {
        ...
        const textScore =  `<div class='score'>
                    Счет игры: ${score - 1}
                </div>
                `
        
        bodyElement.insertAdjacentHTML('afterbegin', textScore);
    },
    ...
    tickHandler() {
        ...
        this.config.setRefreshCounter(this.config.getRefreshCounter() - 1)
        if (this.config.getRefreshCounter() == 0) {
            this.obstacle.setCoordinates(this.getRandomFreeCoordinates());
            this.config.setRefreshCounter(10)
        }
        console.log(this.config.getRefreshCounter());
    ...
    render() {
        ...
        this.renderScore(this.snake.getBody().length);
    }
